...
def main():
    if len(sys.argv) < 2:
        print("Usage: python image_crop.py <image_path(s)> [--custom width height position]")
        return
    ...
    for image_path in image_paths:
        if custom_options:
            if len(custom_options) != 3:
                print("Usage for custom crop: python image_crop.py <image_path(s)> --custom <width> <height> <position>")
                return
            try:
